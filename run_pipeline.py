...
def build_area_similarity_query(
    ...
    top_activity_family_rows: pd.DataFrame,
    top_activity_rows: pd.DataFrame,
    ...
    activity_behavior_context_parts: list[str] = []
    ...
    for value in top_activity_family_rows["attivita_famiglia"].head(2).tolist():
        label = normalize_whitespace(value)
        if label:
            activity_behavior_context_parts.append(label)
    for value in top_activity_rows["attivita_clean"].head(2).tolist():
        label = normalize_whitespace(value)
        if label:
            activity_behavior_context_parts.append(label)
    ...
    for value in top_behavior_rows["comportamento_focus"].head(3).tolist():
        ...
        if label:
            activity_behavior_context_parts.append(label)
    if activity_behavior_context_parts:
        query_parts.extend(activity_behavior_context_parts)
        query_strategy_parts.append("activity_behavior_context")
...
def build_area_similarity_retrieval_artifacts(
    ...
    activity_family_df: pd.DataFrame,
    activity_df: pd.DataFrame,
    ...
    for _, area_row in merged_area_context_df.iterrows():
        ...
        top_activity_family_rows = area_top_rows(activity_family_df, area_name, limit=3)
        top_activity_rows = area_top_rows(activity_df, area_name, limit=3)
        ...
        query_payload = build_area_similarity_query(
            ...
            top_activity_family_rows=top_activity_family_rows,
            top_activity_rows=top_activity_rows,
...
def compose_area_investigation_hypothesis(
    activity_family_rows: pd.DataFrame,
    behavior_family_rows: pd.DataFrame,
    activity_rows: pd.DataFrame,
    event_context_row: pd.Series | None,
) -> str:
    activity_family = normalize_whitespace(activity_family_rows.iloc[0]["attivita_famiglia"]) if not activity_family_rows.empty else None
    ...
    activity_name = normalize_whitespace(activity_rows.iloc[0]["attivita_clean"]) if not activity_rows.empty else None
    ...
    chunks = []
    if behavior_family and activity_name:
        chunks.append(f"Rischio plausibilmente trainato da `{behavior_family}` durante `{activity_name}`.")
    elif behavior_family and activity_family:
        chunks.append(f"Rischio plausibilmente trainato da `{behavior_family}` dentro la famiglia attivita `{activity_family}`.")
    elif activity_family:
        chunks.append(f"Rischio da approfondire nella famiglia attivita `{activity_family}`.")
...
def build_area_investigation_pack(
    ...
    activity_family_path = output_dir / "area_activity_family_deepdive.csv"
    activity_path = output_dir / "area_activity_deepdive.csv"
    ...
    activity_family_df = pd.read_csv(activity_family_path)
    ...
    similarity_report, similar_event_summary_df = build_area_similarity_retrieval_artifacts(
        ...
        activity_family_df=activity_family_df,
        activity_df=activity_df,
    ...
    for _, area_row in merged_area_context_df.iterrows():
        ...
        pack_row = {
            ...
            "top_activity_families_summary": format_named_rows(
                top_activity_family_rows,
                "attivita_famiglia",
                lambda row: f"unsafe {row['unsafe_rate']:.1%}, lift {row['lift_vs_area']:.2f}x",
            ),
            "top_activities_summary": format_named_rows(
                top_activity_rows,
                "attivita_clean",
                lambda row: f"unsafe {row['unsafe_rate']:.1%}, quota area {row['unsafe_share_of_area']:.1%}",
            ),
        ...
        for index, (_, row) in enumerate(top_activity_family_rows.head(3).iterrows(), start=1):
            pack_row[f"top_activity_family_{index}"] = normalize_whitespace(row["attivita_famiglia"])
        for index, (_, row) in enumerate(top_activity_rows.head(3).iterrows(), start=1):
            pack_row[f"top_activity_{index}"] = normalize_whitespace(row["attivita_clean"])
    ...
    for _, row in pack_df.head(top_n_report).iterrows():
        ...
        if normalize_whitespace(row.get("top_activity_families_summary")):
            lines.append(f"- Famiglie attivita driver: {row['top_activity_families_summary']}.")
        if normalize_whitespace(row.get("top_activities_summary")):
            lines.append(f"- Attivita da guardare: {row['top_activities_summary']}.")
